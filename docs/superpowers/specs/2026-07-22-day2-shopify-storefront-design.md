# Day 2 — Shopify Storefront Integration Design

**Status:** Approved, ready for implementation planning.

## Goal

Connect the Day 1 .NET backend to a real Shopify development store: a
...
- **Tunnel:** Cloudflare Quick Tunnel (`cloudflared tunnel --url
  http://localhost:5072`) exposes the local .NET API (from Day 1) on a
...
- **Shopify custom app:** registered via the Partner Dashboard (not a
  ...
  - **Webhook subscription** for the `inventory_levels/update` topic,
    ...
    HMAC signing secret (replaces the test placeholder from Day 1,
...
- **New backend endpoint:** `GET
  ...
  `ProductsController` — none of Day 1's endpoints let the storefront
  ...
  untracked item, consistent with the pattern the Day 1 final review
...
`Product`/`InventoryLog` updated in Azure SQL Edge (same Day 1 code,
...
- Invalid/missing webhook signature: same 401 behavior as Day 1;
...
- Unknown `inventory_item_id` (no matching `Product` row): the new
  ...
  same reasoning as the Day 1 webhook receiver's 200-on-unknown-item
  fix.
...
- No automated tests for the Liquid/JS side — nothing meaningful to
  ...
  Day 1's stored-procedure verification): change a variant's inventory
...
## Global Constraints (carried forward from Day 1's plan + CLAUDE.md)
...
- Exact Shopify CLI commands and App Proxy/webhook-subscription setup
  ...
  hard-specified here — Day 1 showed these environment/platform
